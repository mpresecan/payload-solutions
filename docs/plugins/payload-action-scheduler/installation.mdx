...
Starting a new project? [Payload Stack](/docs/payload-stack) scaffolds all of this — the plugin, a catalogue of actions gated on the product's features, and a run endpoint locked to a cron secret — as one optional step:

```bash
npx create-payload-stack@latest ridgeline --scheduler
```

See [Scheduled actions](/docs/payload-stack/scheduling) for what it writes.
