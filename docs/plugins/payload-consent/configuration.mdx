...
| `processors` | `true` | The processor register. `false` drops the collection, the `/subprocessors` endpoint and the Processors settings tab, and the seeded privacy policy falls back to a categories-only paragraph. |
...
| `processors` | `[]` | Preset keys or `{ key, overrides }`. See [Processors](/docs/plugins/payload-consent/processors#presets). Rows are seeded unverified. |
| `legalPages` | `true` | Seed the three documents. Requires `company` and `legalPages: true`. |
| `documents` | all five | `privacy`, `terms`, `cookies`, `subprocessors`, `dpa` — the last two only when the register is on. Narrow to `['privacy', 'cookies']` if you have your own terms. |
