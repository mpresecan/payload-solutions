...
A tracker is a browser concern: what runs on the page and what the visitor may refuse. Your host, database, email sender and error tracker never touch the banner, so they are not here — they belong in [Processors](/docs/plugins/payload-consent/processors), which is what the privacy policy's recipients table and your sub-processor list are built from.
