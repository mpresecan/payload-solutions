...
Settings for the public sub-processor list: the advance notice period (30 days by default), the address customers object to, an optional subscribe URL, and the read-only `subprocessorsVersion` and its timestamp. Present only when the register is on. See [Processors & DPA](/docs/plugins/payload-consent/processors).
