...
The recording **mode** is deliberately not in the public config — clients only need to know whether to post.
...
| `legalPagesEditor()` | the Lexical editor with tables and both blocks — reuse it in another collection |
| `CookieTableBlock`, `PolicyVersionBlock` | the two blocks |
| `TRACKER_PRESETS` | the raw preset definitions |
| `PURGE_TASK_SLUG` | `'consentPurgeRecords'` |

Types: `ConsentPluginOptions`, `ResolvedConsentPluginOptions`, `ConsentPluginSlugs`, `CompanyInfo`, `SeedOptions`, `TrackerPresetKey`.
...
| Export | What |
| --- | --- |
| `legalPageConverters(data)` | tables **and** both blocks — the one to spread in `RichText` |
| `consentBlockConverters(data)` | the two blocks only |
...
`data` for the converters is `{ categories, trackers, documentsVersion?, effectiveDate?, labels? }`.
