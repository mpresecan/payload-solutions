...
| **Kind** | `privacy`, `terms`, `cookies`, `dpa`, `other`. Only `privacy` and `cookies` feed `documentsVersion`. |
...
With `legalPages` on and `seed.company` set, three documents are created into an empty collection and published:
...
Both are exported as `CookieTableBlock` and `PolicyVersionBlock` if you want them in another collection's editor.
...
`legalPageConverters` is the one to spread: it covers tables and both blocks. If you already have a table converter you are happy with, spread `consentBlockConverters` instead and keep yours. `getCookieTableData(payload)` is a lighter alternative to `getConsentConfig` when all you need is the table's data.
...
`markdownToLegalContent` runs any markdown through the legal-pages editor config — tables included — and swaps the `{{cookie-table}}` and `{{policy-version}}` marker paragraphs for the corresponding blocks. It is the supported way to import an existing policy from markdown.
